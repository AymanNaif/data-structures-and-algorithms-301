...
const sortNumbers = (arr) => {
  ...
  const sortPeopleBetter = (arr) => {
    return arr.sort((frst, sec) => {
      if (frst.lastName < sec.lastName) {
        return -1;
      }
      if (frst.lastName === sec.lastName) {
        if (frst.firstName < sec.firstName) {
          return -1;
        }
      }
      if (frst.firstName === sec.firstName && frst.lastName === sec.lastName) {
        if (frst.age < sec.age) {
          return -1;
        }
      }
  ...
  const sortSchedule = (arr) => {
    ...
    return arr.sort((frstMeeting, scndMeeting) => {
      if (days.indexOf(frstMeeting.dayOfWeek) < days.indexOf(scndMeeting.dayOfWeek)) {
        return -1;
      }
      if (days.indexOf(frstMeeting.dayOfWeek) === days.indexOf(scndMeeting.dayOfWeek)) {
        if (frstMeeting.start === scndMeeting.start) {
          return Number(frstMeeting.end) - Number(scndMeeting.end);
        } else {
          return Number(frstMeeting.start) - Number(scndMeeting.start);
        }
      }
    });
  };
